...
char *DaysRU[7] = {"âîñêðåñåíüå", "ïîíåäåëüíèê", "âòîðíèê", "ñðåäà", "÷åòâåðã", "ïÿòíèöà", "ñóááîòà"};
char *DaysENG[7] = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};

//Months of the year in Russian and English
char *monthsRU[12] = {"ÿíâàðÿ", "ôåâðàëÿ", "ìàðòà", "àïðåëÿ", "ìàÿ", "èþíÿ", "èþëÿ", "àâãóñòà", "ñåíòÿáðÿ", "îêòÿáðÿ", "íîÿáðÿ", "äåêàáðÿ"};
...
void printMainMenu()
{
	...
	if (!language)
	{
		LCD_String(6,8,"Ñåêóíäîìåð"); 
		LCD_String(6,80,"Òàéìåð");
	}
	else
	{
...
void printStopwatchMenu()
{
	...
	{
		LCD_String(0,0,"íàçàä");
		LCD_String(5,27,"Ñòîï");
		LCD_String(5,64,"Ñòàðò");
		if (isStartedSWT)
		{
			LCD_String(7,45, "Ñáðîñ");
		}
	}
...
void printTimerMenu()
{
	...
	else //Rendering the timer menu after starting accordig to language
	{
		...
		if (!language) LCD_String(6,45,"Ñáðîñ");
...
void printSettings()
{
	if (!language)//Displaying all buttons in Russian
	{
		...
		LCD_String(2,0,"Ñòèëü äàòû:");
		LCD_String(3,10, "- 1 Ìàÿ");
		LCD_String(4,10, "- 01.05");
		LCD_String(5,10, "- 05.01");
		
		LCD_String(7,0, "ßçûê ñèñòåìû");
		LCD_String(7, 92, "ÐÓ");
